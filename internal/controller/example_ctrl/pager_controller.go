...
// SELECT count(*) FROM item as a left join product as b on a.pid = b.pid left join product_pagedata as c on b.pid = c.pid left join site as d on b.source_site = d.site_id WHERE a.user_id = 681972 AND a.status = 0
...
func (ctrl *PagerController) Pager(c *gin.Context) {
	...
	query := database.DB.
		...
		Joins("left join product_pagedata as c on b.pid = c.pid").
